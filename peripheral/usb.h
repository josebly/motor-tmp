 ...
 public:
    ...
    void send_data32(uint8_t endpoint, const uint32_t *data, uint8_t length32) 
    {
        ...
        if (USBx_INEP(endpoint)->DIEPCTL & USB_OTG_DIEPCTL_EPENA) {          
            ...
            while(!(USBx_INEP(endpoint)->DIEPINT & USB_OTG_DIEPINT_INEPNE) && !(USBx_DEVICE->DSTS & USB_OTG_DSTS_SUSPSTS) && !(USBx_INEP(endpoint)->DIEPCTL & USB_OTG_DIEPCTL_NAKSTS) );    //wait on nak, exit if suspend (disconnect)
